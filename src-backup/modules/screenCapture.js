...
export default class ScreenCapture {
   ...
   * @returns {boolean} True if permissions are granted or not required, false otherwise
   * 
   * @example
   ...
   * @async
   * @param {Object} bounds - The screen area to capture
   * @param {number} bounds.x - X coordinate of the top-left corner
   * @param {number} bounds.y - Y coordinate of the top-left corner  
   * @param {number} bounds.width - Width of the capture area
   * @param {number} bounds.height - Height of the capture area
   * @returns {Promise<Buffer>} PNG image buffer of the captured area
   * @throws {Error} If permissions are not granted or capture fails
   * 
   * @example
   * const capture = new ScreenCapture();
   * const bounds = { x: 100, y: 100, width: 300, height: 200 };
   * try {
   *   const imageBuffer = await capture.captureArea(bounds);
   *   // Process the captured image buffer
   * } catch (error) {
